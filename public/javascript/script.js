// função para calcular o índice de uma data a partir de uma data inicial
function calcularIndice(dataInicial, dataAlvo) {
  const inicio = new Date(dataInicial); // retorno em ms
...
/*==================== COMENTÁRIO GERAL ====================*/
document.addEventListener("DOMContentLoaded", function() {
  function setupGeneralComments(section){
    var generalComments = [];
    var commentSection = document.getElementById(`general_comment_section_${section}`);
    var addCommentButton = document.getElementById(`add_comment_button_${section}`);
    var addCommentSection = document.getElementById(`add_comment_section_${section}`);
    var commentBox = document.getElementById(`general_comment_section_${section}`);
    
    // abrindo a seção dos comentários ao clicar em Comentarios Gerais
    document.getElementById(`open_general_comment_${section}`).addEventListener('click', function() {
    ...
    // mostrar a caixa de input
    document.getElementById(`add_comment_button_${section}`).addEventListener('click', function() {
    ...
    // clicando em Adicionar Comentario para fazer o display dele na area correta
    document.getElementById(`submit_general_comment_${section}`).addEventListener('click', function() {
      addComment();
    });

    // ou apertando enter!
    document.getElementById(`general_comment_input_${section}`).addEventListener('keydown', function(event) {
      if (event.key === 'Enter') {
    ...
    function addComment() {
      var commentInput = document.getElementById(`general_comment_input_${section}`);
      var comment = commentInput.value.trim();
    
      if (comment) {
        generalComments.push(comment); 
    
        displayComment(comment, generalComments.length - 1);
    
        // após adicionar, limpar a caixa de input e esconder ela
        commentInput.value = '';
        document.getElementById(`add_comment_section_${section}`).style.display = 'none';
      }
    }
    ...
    function displayComment(comment, index) {
      ...
      commentDiv.style.backgroundColor = '#e0e0e0';
      ...
      deleteButton.addEventListener('click', function() {
        generalComments.splice(index, 1); // Remove o comentário da lista
        commentDiv.remove(); // Remove o comentário do DOM
      });
...
periodsEnergiaXDemanda.forEach(period => {
  document.querySelector(period.id).addEventListener('click', function(e) {
      resetCssClasses(e);
      chartD.zoomX(new Date(period.start).getTime(), new Date(period.end).getTime());
  });
});
...
// Por que o neet metering ta sumindo?
// Por que o filtro n ta funcionando
