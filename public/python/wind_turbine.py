...
class WindTurbine:
  ...
  Args:
    cost_per_kw (:type:`int | float`): Cost of the wind turbine per unit of rated power in [$/kW].
    ...
    lifetime (:type:`int | float`): The wind turbine lifetime in time intervals.
  ...
  def __init__(self,
    ...
    ''' Total costs of the public grid during the operation simulation in [$]. '''
  ...
  def generate_power(self, wind_speed: npt.NDArray[np.floating], height_reference: int | float) -> None:
    ...
    Args:
        wind_speed (:type:`npt.NDArray[np.floating]`): A numpy array with the wind speed measurements at the reference height.
  ...
  def economic_analysis(self,
    ...
    .. math::
        \text{NPV}_{om} = \sum^{T}_{t=1}\frac{\text{IC}_{wt} \cdot \tau_{om}}{(1 + d)^t}.
    ...
    .. math::
      \text{NPV}_{repl} = \sum^{T}_{t=1}\frac{\left(\left\lfloor \frac{t}{T_{\text{repl}}} \right\rfloor - \left\lfloor \frac{t-1}{T_{\text{repl}}} \right\rfloor\right) \cdot \text{IC}_{wt}}{(1 + d)^t},

    where :math:`T_{repl} = I^{\text{lifetime}}_{\text{wt}}` is the time when the euipament must to be replaced.
