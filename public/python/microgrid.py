...
class Microgrid:
  ...
  Args:
    ...
    photovoltaic_panel (:class:`~photovoltaic_panel.PhotovoltaicPanel` :type:`| None`): A :class:`~photovoltaic_panel.PhotovoltaicPanel` instance. Default is ``None``.
    wind_turbine (:class:`~wind_turbine.WindTurbine` :type:`| None`): A :class:`~wind_turbine.WindTurbine` instance. Default is ``None``.
    battery (:class:`~battery.Battery` :type:`| None`): A :class:`~battery.Battery` instance. Default is ``None``.
    public_grid (:class:`~public_grid.PublicGrid` :type:`| None`): A :class:`~public_grid.PublicGrid` instance. Default is ``None``.
    inverter (:class:`~inverter.Inverter` :type:`| None`): A :class:`inverter.Inverter` instance. Default is ``None``.
    converter (:class:`~converter.Converter` :type:`| None`): A :class:`converter.Converter` instance. Default is ``None``.
  ...
  def __init__(self,
    ...
    ''' A :class:`~photovoltaic_panel.PhotovoltaicPanel` instance. Default is ``None``. '''
    self.wind_turbine: WindTurbine | None
    ''' A :class:`~wind_turbine.WindTurbine` instance. Default is ``None``. '''
    self.battery: Battery | None
    ''' A :class:`~battery.Battery` instance. Default is ``None``. '''
    self.public_grid: PublicGrid | None
    ''' A :class:`~public_grid.PublicGrid` instance. Default is ``None``.'''
    self.inverter: Inverter | None
    ''' A :class:`inverter.Inverter` instance. Default is ``None``.'''
    self.converter: Converter | None
    ''' A :class:`converter.Converter` instance. Default is ``None``.'''
    self.hour_steps: int
    ''' Number of hour steps in the  '''
  ...
  def initialize(self, max_pan: int | float, max_wind: int | float) -> None:
    ...
    self.surplus_energy = np.zeros(self.hour_steps)
    self.lcoe = 0.0
    self.renewable_factor = 0.0
    self.meef = 0.0
    # Initialization of microgrid components
    if self.photovoltaic_panel:
      self.photovoltaic_panel.initialize(self.hour_steps, max_pan)
    if self.wind_turbine:
      self.wind_turbine.initialize(self.hour_steps, max_wind)
    if self.battery:
  ...
  def run(self, max_pan: int | float, max_wind: int | float) -> npt.NDArray[np.floating]:
    ''' Runs the Microgrid  '''

    # Initialize the microgrid components
    self.initialize(max_pan, max_wind)
    # Generates energy by generators
