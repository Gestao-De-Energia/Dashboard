...
export default async function runMicrogrid(
    ...
    for (let i = 0; i < NPOP; i++) {
        ...
        const vmax = (UB[0]-LB[0])/NPOP;
        const vmin = -vmax;

        for (let y = 0; y < 2; y++) {
            const vel = Math.random() * (vmax - vmin) + vmin;
            particle[i].velocity.push(vel);
        }
    ...
    function inertia(vel) {
        const w = 0.5;
        return vel.map((v) => w * v);
    }

    function cognitive(pbest, x) {
        const c1 = 0.5;
        return pbest.map((p, i) => {
            let rand = Math.random();
            return c1 * rand * (p - x[i]);
        });
    }

    function social(gbest, x) {
        const c2 = 0.8;
        return gbest.map((g, i) => {
            let rand = Math.random();
            return c2 * rand * (g - x[i]);
        });
    }
    ...
    for (let u = 0; u < max_it; u++) {
        ...
        for (let i = 0; i < NPOP; i++) {
            let ine = inertia(particle[i].velocity);
            let c = cognitive(particle_bests[i].position, particle[i].position);
            let s = social(globalbest.position, particle[i].position);

            for (let y = 0; y < 2; y++) {
                particle[i].velocity[y] = ine[y] + c[y] + s[y];
                particle[i].position[y] = particle[i].position[y] + particle[i].velocity[y];
                
                // Lidando com os limites de LB e UB
            ...
            if (particle[i].fitness < particle[i].best.fitness) {
                ...
                // Atualiza também o array particle_bests para acompanhar a melhor posição
                particle_bests[i].cost = particle[i].cost;
                particle_bests[i].rf = particle[i].rf;
                particle_bests[i].meef = particle[i].meef;
                particle_bests[i].position = [...particle[i].position];
                particle_bests[i].velocity = [...particle[i].velocity];
                particle_bests[i].fitness = particle[i].fitness;
